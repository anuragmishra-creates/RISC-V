...
{
    return (imm << 20) | (rs1 << 15) | (spec.func3 << 12) | (rd << 7) | (spec.opcode);
}
...
{
    return (spec.func7 << 25) | (shamt << 20) | (rs1 << 15) | (spec.func3 << 12) | (rd << 7) | (spec.opcode);
}
...
int main()
{
    ...
    cout << "Reading the RISC-V Assembly Code from the \"input.txt\"\n";
    ...
    while (getline(infile, line))
    {
        ...
        if (tokens[0].back() == ':')
        {
            ...
            if (tokens.empty())
                continue; // label only line was there
        }
        allLines.push_back(filtered);
        pc += 4;
    }

    // Second pass to generate the instructions of size 32 bits each:
    cout << "The corresponding Machine Code for the given RISC-V Assembly Code is:\n";
    pc = 0;
    for (auto &l : allLines)
    {
        uint32_t machineCode;
        if (parseInstructionLine(l, machineCode, pc))
            cout << bitset<32>(machineCode) << "\n";
        pc += 4;
    }
    return 0;
}